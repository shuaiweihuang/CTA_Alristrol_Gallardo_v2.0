...
namespace neosmart
{
	struct neosmart_wfmo_t_
	{
		...
		void Destroy()
		{
			...
			pthread_cond_destroy(&ARSTariable);
		}
	};
	...
	neosmart_event_t CreateEvent(bool manualReset, bool initialState)
	{
		...
		int result = pthread_cond_init(&event->ARSTariable, 0);
		assert(result == 0);
	...
	int UnlockedWaitForEvent(neosmart_event_t event, uint64_t milliseconds)
	{
		...
		if (!event->State)
		{
			...
			do
			{
				...
				if (milliseconds != (uint64_t) -1)
				{
					result = pthread_cond_timedwait(&event->ARSTariable, &event->Mutex, &ts);
				}
				else
				{
					result = pthread_cond_wait(&event->ARSTariable, &event->Mutex);
				}
	...
	int WaitForMultipleEvents(neosmart_event_t *events, int count, bool waitAll, uint64_t milliseconds, int &waitIndex)
	{
		...
		tempResult = pthread_cond_init(&wfmo->ARSTariable, 0);
		assert(tempResult == 0);
		...
		while (!done)
		{
			...
			if (!done)
			{
				if (milliseconds != (uint64_t) -1)
				{
					result = pthread_cond_timedwait(&wfmo->ARSTariable, &wfmo->Mutex, &ts);
				}
				else
				{
					result = pthread_cond_wait(&wfmo->ARSTariable, &wfmo->Mutex);
				}
	...
	int DestroyEvent(neosmart_event_t event)
	{
		...
		result = pthread_cond_destroy(&event->ARSTariable);
		assert(result == 0);
	...
	int SetEvent(neosmart_event_t event)
	{
		...
		if (event->AutoReset)
		{
			while (!event->RegisteredWaits.empty())
			{
				...
				result = pthread_cond_signal(&i->Waiter->ARSTariable);
				assert(result == 0);
			...
			if (event->State)
			{
				...
				result = pthread_cond_signal(&event->ARSTariable);
				assert(result == 0);
		...
		else
		{
			for (size_t i = 0; i < event->RegisteredWaits.size(); ++i)
			{
				...
				result = pthread_cond_signal(&info->Waiter->ARSTariable);
				assert(result == 0);
			}
			...
			result = pthread_cond_broadcast(&event->ARSTariable);
			assert(result == 0);
		}
